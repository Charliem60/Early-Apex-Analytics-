...
# Air and Track Temperature
    ...
    axes[0].set_title(f"{year} {event} {session_type} Weather Report", color = "yellow", fontweight = "bold")
    axes[0].legend()
    axes[0].grid()
# Humidity
    axes[1].plot(session_time, weather["Humidity"], color="red", linewidth=2, label="Humidity")
    axes[1].set_ylabel("Humidity (%)")
    axes[1].grid()
# Pressure
    axes[2].plot(session_time, weather["Pressure"], color="orange", linewidth=2, label = "Pressure")
    axes[2].set_ylabel("Pressure (hPa)")
    axes[2].grid()
# Wind Speed
    axes[3].plot(session_time, weather["WindSpeed"], color = "lime", linewidth=3, label="Wind Speed")
    axes[3].set_ylabel("Wind Speed (m/s)")
    axes[3].legend()
    axes[3].grid()
# Rainfall
    axes[4].plot(session_time, weather["Rainfall"].astype(int), color="darkblue", linewidth=3, label="Rainfall")
